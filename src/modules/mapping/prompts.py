...
- Do not use `isNull()`, `isNil()`, or `isNill()`.
- Use `isPresent(value)` for structured-data presence checks when it expresses the rule more clearly than `has(...)`.
- Do not generate `nil` in new expressions unless error context explicitly shows that `null` is not accepted.
- MEL conditionals must not mix `null` with a concrete branch type, e.g. avoid `foo == null ? null : foo.uc()` because the branches are `(null, string)`.
...
- For fixed text additions where null input should stay null, prefer null-safe `.prefix(text)` and `.suffix(text)`, e.g. `title.suffix(' (suffix)')`.
- If absent input should produce an empty string, use a string fallback such as `title == null ? '' : title.suffix(' (suffix)')`.
...
- MEL exposes Unix epoch seconds for timestamps through `.getEpochSecond()` and epoch milliseconds through `.getEpochMillisecond()`. Use these only when examples clearly require epoch seconds, epoch milliseconds, epoch minutes, or an epoch-based numeric value.
- Use `.getNanos()` only when examples clearly require the nanosecond component.
...
- Prefer `.strftime(format)` for formatting timestamps to strings and `.strptime(format)` for parsing strings to timestamps.
- `.formatDateTime()` and `.parseDateTime()` are available for Java-style format compatibility; use them only when the examples clearly use Java date/time patterns.
- Never transform timestamps to strings with `stringify()` or `str()`. Use timestamp formatting helpers instead.
...
- Conditional: `condition ? valueIfTrue : valueIfFalse`
  ...
  - Do not mix `null` with string, number, or boolean branches.
...
- Do not use `isNull()`, `isNil()`, or `isNill()`.
- Use `isPresent(value)` for structured-data presence checks when it is clearer than `has(...)`.
...
- Method calls on null values are safe for common MEL string functions:
  ...
  - `foo.prefix('X')` and `foo.suffix('Y')` return null when `foo == null`.
...
- Java/Groovy string regex methods are invalid:
  - Invalid: `.replaceAll()` and `.replaceFirst()`
  - Correct: use MEL regex helpers such as `.reReplace(...)`, `.reFind(...)`, `.reFindAll(...)`, and `.reMatches(...)` when examples require regex behavior.
...
- `.prefix(prefix)`, `prefix(value, prefix)`: Add prefix to string; null-safe.
- `.suffix(suffix)`, `suffix(value, suffix)`: Add suffix to string; null-safe.
...
- `.reMatches(...)`, `reMatches(...)`: Regex match helpers.
- `.reFind(...)`, `.reFindAll(...)`: Regex search and extraction helpers.
- `.reReplace(...)`: Regex replacement helper.
...
- `.getEpochSecond()`: Convert a timestamp to Unix epoch seconds.
- `.getEpochMillisecond()`: Convert a timestamp to Unix epoch milliseconds.
- `.getNanos()`: Get the nanosecond component.
- `int(timestampValue)`: Convert a timestamp to Unix epoch seconds when needed for compatibility.
...
- `.formatDateTime(format)`: Format using Java SimpleDateFormat notation. Available for compatibility.
- `.parseDateTime(format)`: Parse using Java SimpleDateFormat notation. Available for compatibility.
...
- `isPresent(value)`: Check whether a structured-data value is present.
- `.findItem(path)`: Find an item by path in a structured object, e.g. `focus.findItem('activation/administrativeStatus')`.
- Assignment helpers:
  - `assignment.hasRelation(relation)`, `assignment.hasDefaultRelation()`, `assignment.hasOwnerRelation()`
  - `assignment.isTarget(oid)`, `assignment.isTargetRole()`, `assignment.isTargetOrg()`, `assignment.isTargetService()`, `assignment.isTargetPolicy()`, `assignment.isTargetUser()`
  - `assignment.targetOid()`, `assignment.targetRelation()`, `assignment.targetType()`
...
- Use `foo == null` and `foo != null` for null comparisons. Use `isPresent(value)` for structured-data presence checks when it is clearer than `has(...)`. Do not generate `isNull()`, `isNil()`, or `isNill()`.
- Do not generate `nil` unless error context explicitly requires it.
- Ternary branches must have the same practical type. Do not generate forms like `input == null ? null : input.uc()` or `validTo == null ? null : string(...)`; they fail as `(bool, null, string)`.
...
- If examples require true null in a string transformation, prefer a naturally null-safe function such as `.prefix(...)`, `.suffix(...)`, `.uc()`, or `.lc()` instead of a ternary.
...
- Prefer `.prefix(...)` and `.suffix(...)` for fixed text addition when null input should stay null.
- Regex checks are available through `.matches(regex)`, `matches(string, regex)`, `.reMatches(...)`, and `reMatches(...)`.
- Regex extraction/replacement helpers `.reFind(...)`, `.reFindAll(...)`, and `.reReplace(...)` are available. Use them when examples require regex extraction or regex replacement.
- For character-level extraction such as keeping only digits, use `.reFindAll('[0-9]').join('')` when available.
- Use `.replace(search, replacement)` only for literal replacement. Use `.reReplace(...)`, `.reFind(...)`, or `.reFindAll(...)` when examples require regex replacement or regex extraction.
...
- Epoch seconds are available as `timestampValue.getEpochSecond()`. For ISO/RFC3339 strings with timezone, use `timestamp(value).getEpochSecond()`. For timestamp-like strings without timezone that examples treat as UTC, append `'Z'` before conversion, e.g. `timestamp(validTo + 'Z').getEpochSecond()`.
- Epoch milliseconds are available as `timestampValue.getEpochMillisecond()`. Nanoseconds are available as `timestampValue.getNanos()`. Do not invent `.seconds` or `.epochSecond()`, and do not misuse `.getSeconds()` for epoch conversion.
- Assignment helpers are available for assignment expressions; prefer them over manual `targetRef` field checks when they express the rule directly, e.g. `assignment.isTargetRole()` or `assignment.hasOwnerRelation()`.
- Use nested ternary expressions with explicit parentheses, e.g. `a ? (b ? c : d) : e`.
...
- Null-preserving suffix: `title.suffix(' (suffix)')`
- Blank fallback suffix: `title == null ? '' : title.suffix(' (suffix)')`
...
- Epoch minutes from UTC-like local string: `timestamp(validTo + 'Z').getEpochSecond() / 60`
- Epoch-based `.0` string: `string(11644473600000 + validTo.getEpochSecond()) + '.0'`
- Assignment target role check: `assignment.isTargetRole()`
- Assignment owner relation check: `assignment.hasOwnerRelation()`
""".strip()
...
If error context includes a previous script, fix it or rewrite it so that it matches the examples and remains valid MEL.
...
- Do not use `isNull()`, `isNil()`, or `isNill()`.
- Use `isPresent(value)` for structured-data presence checks when it is clearer than `has(...)`.
- Do not use `nil` unless error context explicitly requires it.
- Do not mix `null` with a string, number, or boolean branch in a ternary. For string outputs with blank fallback, use `value == null ? '' : stringExpression`. If the required fallback is true null, prefer a naturally null-safe expression such as `.prefix(...)`, `.suffix(...)`, `.uc()`, or `.lc()`.
...
- Use `.prefix(...)` and `.suffix(...)` for fixed text addition when null input should stay null.
...
- Use regex boolean checks with `.matches(...)`, `matches(...)`, `.reMatches(...)`, or `reMatches(...)`.
- Use `.reFind(...)`, `.reFindAll(...)`, and `.reReplace(...)` when examples require regex extraction or regex replacement.
- For digit extraction, use `value.reFindAll('[0-9]').join('')`.
...
- If expected outputs are whole-number strings with a `.0` suffix, preserve that suffix exactly. For epoch seconds use `.getEpochSecond()`; for epoch milliseconds use `.getEpochMillisecond()`. Do not invent unsupported timestamp epoch helpers such as `.seconds` or `.epochSecond()`. Do not use `.getSeconds()` for epoch conversion or date-difference calculations; it is only the second-of-minute component.
- For assignment values, prefer assignment helper functions such as `.isTargetRole()`, `.hasOwnerRelation()`, `.targetOid()`, `.targetRelation()`, and `.targetType()` over manual nested `targetRef` checks when they express the rule directly.
